...
int main(void)
{
  ...
  /* Let the MCP2515 stabilise after power-on */
  HAL_Delay(100);

  /* Initialise MCP2515: reset, configure 500 kbps and switch to Normal mode */
  MCP2515_Init();

  /* Prepare the CAN frame sent periodically */
  tx_frame.id = 0x100;     /* 11-bit standard identifier */
  tx_frame.dlc = 1;        /* 1 data byte */
  tx_frame.data[0] = 0xAA; /* Command: activate LED + buzzer on slave */
